...
LearningModelDevice getLearningModelDeviceFromAdapter(com_ptr<IDXCoreAdapter> spAdapter);
...
int main(int argc, char* argv[])
{
	...
	if (!dxCoreDll)
	{
		printf("DxCore isn't supported on this machine! Exiting...");
	...
	{
		printf("Couldn't get the funciton: DXCoreCreateAdapterFactory! Exiting...");
		return HRESULT_FROM_WIN32(GetLastError());
	}
	...
	HRESULT hr = dxcoreCreateAdapterFactory(IID_PPV_ARGS(spFactory.put()));
	if (FAILED(hr))
	{
		wprintf(hresult_error(hr).message().c_str());
		return hr;
	}
	...
	CHAR driverDescription[MAX_PATH];
	...
	for (UINT i = 0; i < spAdapterList->GetAdapterCount(); i++)
	{
		...
		hr = spAdapterList->GetItem(i, spAdapter.put());
		if (FAILED(hr))
		{
			wprintf(hresult_error(hr).message().c_str());
			return hr;
		...
		spAdapter->QueryProperty(DXCoreProperty::IsHardware, sizeof(isHardware), &isHardware);
		spAdapter->GetHardwareID(&dxCoreHardwareID);
	...
	if (validAdapters.size() == 0) {
		...
		hr = spAdapterList->GetItem(selectedIndex, selectedAdapter.put());
		selectedAdapter->QueryProperty(DXCoreProperty::DriverDescription, sizeof(driverDescription), driverDescription);
		printf("Selected adapter at index %d, description: %s\n", selectedIndex, driverDescription);
		try
		{
			device = getLearningModelDeviceFromAdapter(validAdapters[selectedIndex]);
		}
